...
public class CharacterViewer : MonoBehaviour {
	...
	void Update () {
        ...
        if (Input.GetAxis("Mouse ScrollWheel")!=0)
        {
            ...
            if (Input.GetAxis("Mouse ScrollWheel") < 0 && deltaPosition.y < 13) y = Input.GetAxis("Mouse ScrollWheel");
            if (Input.GetAxis("Mouse ScrollWheel") > 0 && deltaPosition.y > 3f) y = Input.GetAxis("Mouse ScrollWheel");
